...
class MonteCarloPredictor(BasePredictor):
    ...
    def predict_paths(self, data, days=30, iterations=50, method="gbm", drift_adj=0.0, volatility_adj=0.0):
        ...
        if method == "jump_diffusion":
            ...
            # Default to GBM
            return self._simulate_gbm(data, days, iterations, drift_adj, volatility_adj)

    def _simulate_gbm(self, data, days, iterations, drift_adj=0.0, volatility_adj=0.0):
        last_price = data['Close'].iloc[-1]
        
        # Adjust drift and volatility
        # drift_adj is additive percentage (e.g. 0.05 for +5%)
        # volatility_adj is multiplier (e.g. 1.2 for +20%)
        
        adj_drift = self.drift + drift_adj
        adj_stdev = self.stdev * (1 + volatility_adj)
        
        # Ensure volatility doesn't go negative
        adj_stdev = max(0.001, adj_stdev)

        # Method 2: Explicit definition (Brownian Motion)
        dt = 1 # 1 day step
        
        # Generates random table of shock
        Z = np.random.normal(0, 1, (days, iterations))
        ...
        for t in range(1, days):
             ...
             drift_term = (adj_drift - 0.5 * adj_stdev**2) * dt
             shock_term = adj_stdev * np.sqrt(dt) * Z[t]
             
             price_paths[t] = price_paths[t-1] * np.exp(drift_term + shock_term)
             
        mean_path = price_paths.mean(axis=1)
        
        # Dates
        last_date = data['Date'].iloc[-1]
        future_dates = [last_date + datetime.timedelta(days=x) for x in range(days)]
             
        return future_dates, mean_path, price_paths.T.tolist()
